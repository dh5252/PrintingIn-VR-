...
public class MinusButton : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        if (_interactable == null)
            Debug.LogError($"[{name}] MinusButton: XRBaseInteractable 컴포넌트가 없습니다.");
        else
        {
        ...
        if (_renderer == null)
            Debug.LogError($"[{name}] MinusButton: Renderer가 없습니다. 버튼 모델에 MeshRenderer를 붙이세요.");
        

        additionalNumber = transform.parent.Find("AdditionalNumber").GetComponent<AdditionalNumber>();
        if (additionalNumber == null)
            Debug.LogError($"[{name}] MinusButton: additionalNumber를 붙이세요.");
